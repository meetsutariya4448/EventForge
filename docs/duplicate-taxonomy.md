...
## Completion check — as of the end of this M1 addendum session

**Rows 6 and 7 are still Predicted.** That's correct and expected: M2 hasn't been built yet, and
this document is not claiming otherwise. Everything within M1's actual scope (rows 1–5) is Proven,
including a negative control on row 5's test (see its Proven-by cell) — the multi-worker test was
confirmed to actually fail without the real fix, not merely pass by construction.

## What this means for M2

Rows 6 and 7 are M2's actual scope. When M2 lands:
- Each row's Status flips from Predicted to Proven, with its "Proven by" cell filled in with the
  real test name(s) — not before, and not by editing the Status alone without a real test to point
  at.
- Any new duplicate mechanism M2's own testing surfaces that doesn't map onto an existing row gets
  a **new row**, added before the fix that addresses it — this table is not allowed to fall behind
  what's actually been discovered.
- If M2 ends with either row still Predicted, M2's stop-and-report must say so explicitly, the same
  requirement this document places on every milestone that touches it.
